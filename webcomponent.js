(function() {
    ...
    function loadthis(that) {
        ...
        if (that._firstConnection === 0) {
            ...
            div0.innerHTML = '<?xml version="1.0"?><script id="oView_' + widgetName + '" name="oView_' + widgetName + '" type="sapui5/xmlview"><mvc:View controllerName="NetworkGraph.NetworkGraph.controller.View1" xmlns:mvc="sap.ui.core.mvc" displayBlock="true" xmlns="sap.m" xmlns:n="sap.suite.ui.commons.networkgraph" xmlns:layout="sap.suite.ui.commons.networkgraph.layout"> 	<Shell id="shell" appWidthLimited="false"> 		<App id="app"> 			<pages> 				<Page id="page"> 					<content> 						<n:Graph enableWheelZoom="false" nodes="{' + widgetName + '>/nodes}" lines="{' + widgetName + '>/lines}" groups="{' + widgetName + '>/groups}" orientation="TopBottom" id="graph"> 							<n:layoutData> 								<FlexItemData minWidth="75%" maxWidth="75%"/> 							</n:layoutData> 							<n:layoutAlgorithm> 								<layout:LayeredLayout 								mergeEdges="{settings>/mergeEdges}" 								nodePlacement="{settings>/nodePlacement}" 								nodeSpacing="{settings>/nodeSpacing}" 									lineSpacingFactor="{settings>/lineSpacingFactor}"/> 							</n:layoutAlgorithm> 							<n:nodes> 								<n:Node height="{settings>/height}" attributes="{path:'attributes', templateShareable:true}" key="{' + widgetName + '>key}" title="{' + widgetName + '>title}" icon="{' + widgetName + '>icon}" 									group="{' + widgetName + '>group}" shape="Box" status="{' + widgetName + '>status}" x="{' + widgetName + '>x}" y="{' + widgetName + '>y}" showDetailButton="false"> 									<n:actionButtons> 										<n:ActionButton icon="sap-icon://wrench" press="onPressNodeActionButton"/> 									</n:actionButtons> 									<n:attributes> 										<n:ElementAttribute label="{' + widgetName + '>label}" value="{' + widgetName + '>value}"/> 									</n:attributes> 								</n:Node> 							</n:nodes> 							<n:lines> 								<n:Line from="{' + widgetName + '>from}" to="{' + widgetName + '>to}" status="{' + widgetName + '>status}"></n:Line> 							</n:lines> 							<n:groups> 								<n:Group key="{' + widgetName + '>key}" title="{' + widgetName + '>title}"></n:Group> 							</n:groups> 						</n:Graph> 					</content> 				</Page> 			</pages> 		</App> 	</Shell> </mvc:View></script>';
